...
class LLMAugmenter:
    ...
    """
    def __init__(self, model_name="meta-llama/Llama-2-7b-chat-hf", temperature=0.3, factor=9) -> None:
    ...
    def get_chat(self, text: str, label: float) -> str:
        system_prompt = "You are an advanced AI writer. Your job is to help write examples of text with a certain sentiment. The examples should have the aim to differ from the input text. Sentiment is represented as a score between 0 and 1, where 0 means it has a very negative effect on the stock market price and 1 means it has a very positive effect on the stock market price."
        task = f"Based on the following news article title which has a {label} sentiment score, write {self.factor} new similar examples in style of a news title, that has the same sentiment. Separate the texts by newline and enumerate them with incremental numbers.",
        chat = [
            {"role": "system", "content": system_prompt},
            {
                "role": "user",
    ...
    def augment_batch(self, instances, text_column="text"):
        inputs = instances.apply(lambda row: self.get_chat(row[text_column], row["target_scaled"]), axis=1).tolist()

        # TODO: Check different batch sizes. Somehow it fails sometimes for batch sizes over 1.
        outputs = self.pipe(inputs, batch_size=1)
        
        answers = "\n".join([output[0]["generated_text"][-1]["content"] for output in outputs])

        pattern = r'^\d+\.\s+(.+)'
        augmentations = re.findall(pattern, answers, re.MULTILINE)
        
        expanded_instances = instances.loc[instances.index.repeat(self.factor)]
        # TODO: SOmetimes the length does not match: This is because some answers dont list their versions in an enumerated way
        expanded_instances[text_column] = augmentations
        expanded_instances["augmented"] = True
        
        return expanded_instances

    def augment(self, text, score):
        input = self.get_chat(text, score)
        output = self.pipe(input)

        answer = output[0]["generated_text"][-1]["content"]

        pattern = r'^\d+\.\s+(.+)'
        examples = re.findall(pattern, answer, re.MULTILINE)
...
if __name__ == "__main__":
    # Example usage
    text = "Apple is facing delivery issues for the new iPads"
    score = 0.2
    texts = [text, "Apple is facing delivery issues for the new iPads"]
    scores = [score, 0.14]
    ...
    nmt_augmenter = BackTranslation("en", "de", True)
    augmented_text = nmt_augmenter.augment(text)
    print("Back Translation:", augmented_text)

    llm_augmenter = LLMAugmenter(factor=5)
    df = pd.DataFrame({"text": texts, "target_scaled": scores})
    augmented_text = llm_augmenter.augment_batch(df)
    print("LLM Augmentation:\n", augmented_text.values)
